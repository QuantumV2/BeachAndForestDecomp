function scr_playersounds() {
    with (obj_player1)
    {
        if (state == (77 << 0) && (!audio_is_playing(sfx_mach1)) && grounded)
            mach1snd = audio_play_sound(sfx_mach1, 1, false)
        else if (state != (77 << 0) || (!grounded) || move == (-xscale))
            audio_stop_sound(mach1snd)
        if ((sprite_index == spr_mach || state == (24 << 0)) && (!audio_is_playing(sfx_mach2)))
            mach2snd = audio_play_sound(sfx_mach2, 1, false)
        else if (sprite_index != spr_mach && state != (24 << 0))
            audio_stop_sound(mach2snd)
        if ((state == (92 << 0) || sprite_index == spr_mach3boost) && sprite_index != spr_crazyrun && (!audio_is_playing(sfx_mach3)))
            mach3snd = audio_play_sound(sfx_mach3, 1, false)
        else if ((state != (92 << 0) && sprite_index != spr_mach3boost) || sprite_index == spr_crazyrun)
            audio_stop_sound(mach3snd)
        if (state == (25 << 0) && (!audio_is_playing(sfx_knightslide)))
            knightslide = audio_play_sound(sfx_knightslide, 1, false)
        else if (state != (25 << 0) && audio_is_playing(sfx_knightslide))
        ...
        if (state != (73 << 0) && audio_is_playing(superjumpprepsnd))
            audio_stop_sound(superjumpprepsnd)
        if (state != (73 << 0) && audio_is_playing(superjumpholdsnd))
        ...
        if (audio_is_playing(suplexdashsnd) && state != (29 << 0))
    ...
    with (obj_player2)
    {
        ...
        if (sprite_index == spr_crazyrun && (!audio_is_playing(sfx_mach4)))
            ...
            audio_stop_sound(mach4snd)
        if (state != (73 << 0) && superjumpprepsnd)
            audio_stop_sound(superjumpprepsnd)
        if (state != (73 << 0) && audio_is_playing(superjumpholdsnd))
            audio_stop_sound(superjumpholdsnd)
        if (sprite_index == spr_tumblestart && (!audio_is_playing(sfx_tumble1)) && floor(image_index) < 11)
