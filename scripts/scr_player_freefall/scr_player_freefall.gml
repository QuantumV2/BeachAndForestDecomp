function scr_player_freefall() {
    ...
    if (grounded && (!(input_buffer_jump < 8)) && (!(place_meeting(x, (y + 1), obj_destructibles))))
    {
        ...
        with (obj_baddie)
        {
            if (grounded && point_in_rectangle(x, y, __view_get((0 << 0), 0), __view_get((1 << 0), 0), (__view_get((0 << 0), 0) + __view_get((2 << 0), 0)), (__view_get((1 << 0), 0) + __view_get((3 << 0), 0))) && (!invincible) && groundpound)
            {
                state = (109 << 0)
                if (stunned < 60)
                    stunned = 60
